module Adapters
  class Connection
    ...
    def self.save_config(name, hash)
      hash.each do |k, v|
        Support::Config.create!(:name => name.to_s, :key => k.to_s, :value => v.to_s)
      end
    end

    def self.load_config(name)
      RecursiveOpenStruct.new(Hash[Support::Config.where(:name => name.to_s).map {|item| [item.key.to_sym, item.value]}])
    end
  end
end
